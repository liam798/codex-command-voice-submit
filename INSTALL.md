# 在另一台 Mac 安装 Codex Voice Auto Send
...
```text
codex-voice-auto-send-<version>.tar.gz
```

对方解压：

```bash
tar -xzf codex-voice-auto-send-<version>.tar.gz
cd codex-voice-auto-send-<version>
```
...
tar -czf codex-voice-auto-send.tar.gz codex-voice-auto-send
```

在目标电脑解压：

```bash
tar -xzf codex-voice-auto-send.tar.gz
cd codex-voice-auto-send
```
...
```text
/Volumes/Disk_APFS/Work/AI/AiDemos/codex-voice-auto-send/dist/codex-voice-auto-send-20260529-183000.tar.gz
```
...
```text
dist/codex-voice-auto-send-v1.0.0.tar.gz
```
...
```text
~/.local/bin/codex-voice-auto-send
~/Library/LaunchAgents/com.local.codex-voice-auto-send.plist
```
...
如果列表里出现 `codex-voice-auto-send`，打开它；如果没有出现，可以先运行一次：

```bash
~/.local/bin/codex-voice-auto-send --request-permission
```
...
```bash
launchctl unload ~/Library/LaunchAgents/com.local.codex-voice-auto-send.plist 2>/dev/null || true
launchctl load ~/Library/LaunchAgents/com.local.codex-voice-auto-send.plist
```
...
```bash
launchctl list | grep codex-voice-auto-send
```

看到类似输出表示已运行：

```text
12345   0   com.local.codex-voice-auto-send
```
...
```bash
tail -80 ~/Library/Logs/codex-voice-auto-send.err.log
tail -80 ~/Library/Logs/codex-voice-auto-send.log
```
...
```bash
launchctl unload ~/Library/LaunchAgents/com.local.codex-voice-auto-send.plist 2>/dev/null || true
rm -f ~/Library/LaunchAgents/com.local.codex-voice-auto-send.plist
rm -f ~/.local/bin/codex-voice-auto-send
```
